...
const filteredVideos = computed(() => {
  ...
  return admin.videos.filter((v) => v.title.toLowerCase().includes(q))
})
...
const handleSave = async () => {
  ...
  if (isEditing.value) {
    // 编辑：发送 JSON 更新 title / url / productId
    const data = {
      title: form.value.title,
      url: form.value.url || null,
      productId: form.value.productId || null,
    }
    const res = await admin.updateVideo(editingId.value, data)
    if (res.code === 200) {
    ...
    const res = await admin.addVideo(fd)
    if (res.code === 200) {
...
const handleDelete = (video) => {
  ElMessageBox.confirm(
    ...
    const res = await admin.deleteVideo(video.id)
    if (res.code === 200) {
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <template v-if="!isEditing">
          ...
          <el-form-item v-else label="视频链接" prop="url">
