...
本项目为团队项目。Git 作者 `HaiMingQAQ` 的主要贡献包括：

- 负责 PC Web 与 uni-app 移动端开发，完成商品、购物车、订单、用户资料、地址、收藏和附近门店等页面与交互。
- 编写完整后端需求说明和各模块修改方案，参与后端代码调整、问题定位与前后端接口联调。
- 编写 `AIService` 对接 DeepSeek API，注入商品上下文，并配合前端解析推荐标记和展示商品卡片。
- 完成 H5/Android 构建、前端静态资源整合、外网联调，以及项目说明、接口与部署文档。
- Git 全部分支历史中累计提交 40+ 次，参与各业务模块的问题修复和版本交付。
